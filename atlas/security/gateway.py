...
class PromptGuard:
    ...
    PATTERNS = (
        ...
        ("credential_exfiltration", re.compile(
            r"(api[_ -]?key|secret|password|private key).{0,40}(send|print|reveal|export)", re.I
        )),
        ("unsafe_tool_chaining", re.compile(
            r"(disable|bypass).{0,30}(sandbox|policy|security)", re.I
        )),
    )
